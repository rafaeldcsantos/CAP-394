...
supermDF = pd.read_csv("Resources/Data/SupermarketCSV/transactions.csv.gz",
                        encoding="iso-8859-1",compression='gzip')
```
...
It is possible that more information on the transactions may be extracted from the original data source, a set of XML files, one per transaction, available as two ZIP files: [part 1, 40 MB](Resources/Data/SupermarketCSV/transactionsXML_1.zip) and [part 2, 33 MB](Resources/Data/SupermarketCSV/transactionsXML_2.zip); which may must be reprocessed and converted to a format that can be ingested into dataframes.
